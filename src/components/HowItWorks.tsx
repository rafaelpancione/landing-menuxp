...
const HowItWorks: React.FC = () => {
  ...
  const PaymentPill: React.FC<{ payment: Order['payment'] }> = ({ payment }) => {
    ...
    const m = map[payment];
    return <span className={`inline-flex items-center px-2 py-0.5 text-[11px] rounded-full border ${m.cls}`}>{m.label}</span>;
  };

  const PlaceBadge: React.FC<{ place: Order['place'] }> = ({ place }) => {
    const icon =
      place.type === 'mesa' ? <UtensilsCrossed className="w-3.5 h-3.5" /> :
      place.type === 'balcao' ? <User className="w-3.5 h-3.5" /> :
        ...
        <div className="p-3 space-y-3">
          ...
          <div className="space-y-1">
            ...
            {order.notes && order.notes.length > 0 && (
              <div className="flex flex-wrap gap-1 pl-6">
                {order.notes.map((n, i) => (
                  <span key={i} className="px-2 py-0.5 rounded bg-yellow-50 border border-yellow-200 text-[11px] font-semibold tracking-wide">
                    {n.toUpperCase()}
                  </span>
                ))}
              </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {order.status === 'pendente' && (
              <button onClick={() => handleAction('start')} className="btn-brut !py-1 !px-2 text-xs inline-flex items-center gap-1">
                <Play className="w-3.5 h-3.5" /> Iniciar
              </button>
            )}
            {['recebido', 'em_preparo'].includes(order.status) && (
              <button onClick={() => handleAction('done')} className="btn-brut !py-1 !px-2 text-xs inline-flex items-center gap-1">
            ...
            {!['entregue', 'cancelado'].includes(order.status) && (
              <button onClick={() => handleAction('cancel')} className="btn-brut white !py-1 !px-2 text-xs inline-flex items-center gap-1">
